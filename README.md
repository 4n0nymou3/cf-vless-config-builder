...
همین محدودیت، طبق مستندات پروژه‌های مشابه، برای **Worker با دامنه اختصاصی (Custom Domain)** هم صدق می‌کند — یعنی حتی اگر روی Worker (نه Pages) یک دامنه اختصاصی متصل کرده باشید، باز هم کانفیگ‌های بدون TLS کار نخواهند کرد. TCB در راهنمای خودش فقط استفاده از آدرس پیش‌فرض `*.workers.dev` را آموزش می‌دهد (بدون دامنه اختصاصی) که این محدودیت را ندارد، اما اگر خودتان به دلخواه یک دامنه اختصاصی روی Worker متصل کرده‌اید، این نکته را در نظر داشته باشید.

اگر به کانفیگ بدون TLS نیاز دارید (مثلاً برای شبکه‌هایی که TLS در آن‌ها بلاک است)، حتماً از روش استقرار **Worker** با همان آدرس پیش‌فرض `*.workers.dev` (بدون دامنه اختصاصی) استفاده کنید.
