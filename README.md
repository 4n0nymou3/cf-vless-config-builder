...
با وارد کردن یک کانفیگ با فرمت URL از یک سرور خارجی (مثلاً یک VPS با IP ثابت) در کادر Chain Proxy، کانفیگ‌های TCB به آن سرور Chain می‌شوند: ابتدا از طریق Worker شما (برای عبور از فیلترینگ) و سپس از طریق آن سرور به اینترنت متصل می‌شوید — یعنی IP خروجی همیشه ثابت و برابر با IP همان سرور خواهد بود.
...
- کانفیگی که در این کادر وارد می‌کنید نباید مربوط به یک Cloudflare Worker دیگر باشد، چون Workerها آی‌پی اختصاصی و ثابت ندارند؛ حتماً از کانفیگ یک سرور واقعی (مثل VPS) با آی‌پی اختصاصی خودش استفاده کنید.
